    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in state.order_depths.keys():
            if product == 'BANANAS':
                ...
                price_history = np.append(price_history, current_avg_market_price)
                if len(price_history) == history_length+1:
                ...
                if state.timestamp > start_trading:
                    model = ARIMA(4,0,1)
                    pred = model.fit_predict(price_history)
                    forecasted_price = model.forecast(pred, 1)
                    forecasted_price = forecasted_price[-1]
